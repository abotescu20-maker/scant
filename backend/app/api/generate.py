...
from app.services.hf_service import generate_video_free
from app.services.storage_service import upload_video, upload_thumbnail, generate_share_code
    ...
    try:
        ...
        if quality == QualityTier.free:
            ...
            video_bytes, _ = await generate_video_free(
                b"", style_id.value, custom_prompt=prompt, progress_cb=_cb,
                animation_mode=animation_mode.value,
                frame_delay=frame_delay,
            )
        else:
            video_bytes = await generate_video_from_image(b"", prompt, quality)
